...
if (isset($_SESSION['username'])) {
    header("Location: assets/pages/loader-cek-jadwal-penerbangan");
}
...
if (isset($_POST['submit'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        header("Location: assets/pages/loader-cek-jadwal-penerbangan?pesan=berhasil-login");
    } else {
        header("Location:loader-masuk-gagal");
    }
}
...
<head>
    ...
    <title>Masuk Dulu! | JWB Digitalent ke-5</title>
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav mx-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link mx-2 active-page" href="assets/php/controller-page?page=1">Cek Jadwal & Penerbangan</a>
                    </li>
                    <li class="nav-item">
    ...
    <div class="container py-3 px-5">
        <div class="row d-flex justify-content-center">
            <div class="col-lg-10 shadow-lg rounded-5 px-5 py-5 position-absolute bg-white">
                <?php
                if ($_GET['pesan']) {
                    if ($_GET['pesan'] == "berhasil-keluar") {
                        echo "
                            <div class='alert alert-primary alert-dismissible fade show' role='alert'>
                            <strong>Berhasil keluar!</strong> tetap semangat dan pastikan akun sudah aman!
                            <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                            </div>
                            ";
                    } elseif ($_GET['pesan'] == "gagal-masuk") {
                        echo "
                        <div class='alert alert-danger alert-dismissible fade show' role='alert'>
                        <strong>Username & Password salah!</strong> silahkan periksa kembali dan masuk lagi!
                        <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                        </div>
                        ";
                    }
                ...
                <form action="" method="post">
                    <h4>Masuk dulu! untuk cek jadwal dan penerbangan!</h4>
                    <div class="mb-3">
                        <label for="">Email</label>
                        <input class="form-control" type="email" placeholder="Email" name="email" value="<?php echo $email; ?>" autofocus required>
                    </div>
                    <div class="mb-3">
                        <label for="">Password</label>
                        <input class="form-control" type="password" placeholder="Password" name="password" value="<?php echo $_POST['password']; ?>" required>
                    </div>
                    <input type="submit" value="Masuk" class="btn-pesan-tiket" name="submit">
                </form>
            </div>
        </div>
    </div>
